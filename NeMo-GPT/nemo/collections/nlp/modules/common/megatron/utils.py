...
def get_grouped_params_for_weight_decay_optimization(
    ...
    further more, divide params into embedding, encoder.layers.{index}, ... blocks; for better metrics logging
    """
    from nemo.collections.nlp.modules.common.megatron.module import Float16Module
    modules = listify_model(model)
    print(modules)
    optimizer_grouped_parameters = []
    no_decay = ['bias', 'LayerNorm', 'Norm']  # gamma/beta are in LayerNorm.weight
    for module in modules:
        module_ = module.module if isinstance(module, Float16Module) else module
        optimizer_grouped_parameters.extend(
            [{'params': [p for n, p in module_.language_model.embedding.named_parameters() if not any(nd in n for nd in no_decay) and p is not None], 'name': 'model.embeddings'},
             {'params': [p for n, p in module_.language_model.embedding.named_parameters() if any(nd in n for nd in no_decay) and p is not None], 'weight_decay': 0.0, 'name': 'model.embeddings'}
             ]
            )
        for encoder_layer_index, encoder_layer in module_.language_model.encoder.layers.named_children():
            optimizer_grouped_parameters.extend(
                [{'params': [p for n, p in encoder_layer.named_parameters() if not any(nd in n for nd in no_decay) and p is not None], 'name': f'model.encoder.layers.{encoder_layer_index}'},
                 {'params': [p for n, p in encoder_layer.named_parameters() if any(nd in n for nd in no_decay) and p is not None], 'weight_decay': 0.0, 'name': f'model.encoder.layers.{encoder_layer_index}'}
                 ]
                )
        optimizer_grouped_parameters.extend(
            [{'params': module_.language_model.encoder.final_layernorm.parameters(), 'weight_decay': 0.0, 'name': f'model.encoder.final_layernorm'}]
            )
        # this is conditioned on model.share_embeddings_and_output_weights=False, otherwise has to remove them from the param list
        optimizer_grouped_parameters.extend(
            [{'params': [p for n, p in module_.language_model.output_layer.named_parameters() if not any(nd in n for nd in no_decay) and p is not None], 'name': 'model.last'},
             {'params': [p for n, p in module_.language_model.output_layer.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'name': 'model.last'}]
            )
